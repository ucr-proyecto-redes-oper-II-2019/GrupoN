/*autores:
Nicole Castillo - B71755, Fabian Jimenez B73902
*/
#include <stdio.h>
#include <stdlib.h>
#include <unistd.h>
...
void timeout(){
  ...
  printf("%s %lf","Ocurrio un timeout\n", segundos);
  exit(0);
}
/*
Efecto:
		      ...
		      leido = fread(&buff_temp,MAXLINE,1,fd);
			     ...
			     if(leido > 0){
			         ...
			         bandera_buffSend_lleno = 1;
						 	...
						 	copiar(buff_temp,interfazSend_buff,MAXLINE);
						 	bandera_buffSend_lleno = 1;
							while(bandera_buffSend_lleno){//espera a que se consuma el ultimo pedazo de archivo
								;
							}
