        ...
        */
        int get_send(type_ptr dest, int num_package, int length){
             /*printf("BUFF get send: \n");
                ...
                fflush(stdout);*/
            int pivote = num_package%size;
            if (nMin <= num_package && ((nMin+size) > num_package) && vector_lleno[pivote]) {
                copy_to_others(dest,vector[pivote],length,0,0);
                return 1;
            }
            return 0;
        ...
        */
        int set_nMin(int num_package){
            ...
            for (int i = nMin; i < (nMin+size); i++) {
                pivot = i%size;
                if (i < num_package && vector_lleno[pivot]) {
                    //delete vector[pivot];
                    //vector[pivot] = 0;
                    borrar(pivot);
                    vector_lleno[pivot] = 0;
                    item_accounter--;
                }
            }
            nMin = num_package;
