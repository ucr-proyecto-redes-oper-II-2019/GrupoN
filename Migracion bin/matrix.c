...
void escribirMatrizEnArchivo(double matriz[15][15]){
  ...
  for(i = 0; i < 15; i++){
    for(j = 0; j < 15; j++){
      fwrite(&matriz[i][j],sizeof(matriz[i][j]),1,fptr);
...
void escribirArchivo(double A[15][15], double B[15][15], double I[15][15], double Temp[15][15],
  ...
  fwrite(&iters,sizeof(int),1,fptr);
  fwrite(&totalprod,sizeof(int),1,fptr);
  fwrite(&det,sizeof(double),1,fptr);
  fwrite(&sdet,sizeof(double),1,fptr);
  fwrite(&c,sizeof(double),1,fptr);
  fwrite(&caso,sizeof(int),1,fptr);
  //printf("caso guardado: %d", caso);
...
void leerDeMatrizEnArchivo(double matriz[15][15]){
  for(int i = 0 ; i < 15 ; i++){
    for(int j = 0; j < 15; j++){
      fread(&matriz[i][j],sizeof(matriz[i][j]),1,fptr);
...
void cargarVariables(int * dimCopy,int * nCopy,int * iCopy,int * jCopy,int * itersCopy,
   ...
   fread(dimCopy,sizeof(int),1,fptr);
   fread(nCopy,sizeof(int),1,fptr);
   fread(iCopy,sizeof(int),1,fptr);
   fread(jCopy,sizeof(int),1,fptr);
   fread(itersCopy,sizeof(int),1,fptr);
   fread(totalprodCopy,sizeof(int),1,fptr);
   fread(detCopy,sizeof(double),1,fptr);
   fread(sdetCopy,sizeof(double),1,fptr);
   fread(cCopy,sizeof(double),1,fptr);
   fread(&caso,sizeof(int),1,fptr);
   //printf("caso leido: %d", caso);
