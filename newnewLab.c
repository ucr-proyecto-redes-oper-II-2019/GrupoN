...
Cola cola_send(WINDOWLEN,1);
Cola cola_recv(WINDOWLEN,1);
...
void print(char buffer[MAXLINE],int size){
	printf("BUFF: ");
	for(int i = 0; i < size; ++i){
		printf("%c ",buffer[i]);
	}
...
void empaquetar(char * buff_a_empaquetar, char * datos){
  set_header(num_paquete,buff_a_empaquetar);
  sacar_datos(buff_a_empaquetar,datos);
}
...
int main() {
  ...
  //ACK = 1 / SN = 0
	...
	bandera_buffRecv_lleno,bandera_buffRecv_FA,cola_send,cola_recv,ack,signal_file_abierto,ip_asignado) 
	{
		...
		if (omp_get_thread_num()==0) {//hilo compartido por receptor y emisor
			...
			if(tipo == 1){ //si es de tipo mandar archivo la direccion se guarda en cliaddr
				...
				int puerto;
				printf("Digite el numero de puerto que desea asignar: ");
				scanf("%6d",&puerto);
			...
			while(1){
				if(!tipo && ip_asignado){
					...
					if(cola_recv.get_recv(elemento_cola,MAXLINE)){ //si se encontraba el paquete en la cola
						insertar_interfaz_receptor(elemento_cola);
						usleep(1);	
					}
				}
			}
		}else if(omp_get_thread_num() == 2){//hilo de receptor
			while(1){
				if(!tipo && ip_asignado){
			...
			FILE* fptr1;
			fptr1 = fopen("archivo_recibido.bin", "ab+");//se abre para modificarlo
			while(1){
				if(!tipo){
		...
		}else if(omp_get_thread_num() == 4){//hilo de emisor
			int stop = 0;
			while(1){
				if(tipo == 1 && signal_file_abierto){
					...
					poner_en_colaSend(); ////Saca lo que hay en interfaz, elimina lo que hay en ella y pone la bandera de lleno en 0. DEvuelve si lo logro poner 
		...
		}else if(omp_get_thread_num() == 5){//hilo de emisor
			while(1){
				if(tipo == 1 && ip_asignado){
					/*char temp[PACKAGE_SIZE];
					set_header(1,temp);
					temp[4] = 'n';
					temp[5] = '\0';
					sendto(sockfd, (const char *)temp, PACKAGE_SIZE, MSG_CONFIRM , (const struct sockaddr *) &cliaddr, sizeof(cliaddr));*/
					//while(cola_send.get_nMin() <= ultimo_consumido_de_interfazSend){
						//	printf("entra while enviar \n" );
					enviar();
							//cola_send.set_nMin(ack);
