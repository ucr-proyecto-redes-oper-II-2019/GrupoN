...
void TCPLite::send_timeout(){
    ...
    while(1){
        ...
        for(int i = 0; i < bolsa_send->get_size(); i++){
            ...
            cliaddr_send.sin_port = htons(static_cast<unsigned short>(req.port));
            cliaddr_send.sin_addr.s_addr = inet_addr(req.IP);
            sendto(sockfd,static_cast<const char *>(req.paquete), REQMAXSIZE, MSG_CONFIRM, reinterpret_cast<const struct sockaddr *>(&cliaddr_send), sizeof(cliaddr_send)); //no se si se manda asi el ack, hay que revisar
...
int TCPLite::receive(){
    ...
    recvfrom(sockfd, static_cast<char *>(paquete), REQMAXSIZE, MSG_WAITALL, reinterpret_cast<struct sockaddr *>(&cliaddr_recv), &len);
    request r;
