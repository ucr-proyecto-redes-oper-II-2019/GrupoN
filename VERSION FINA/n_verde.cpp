...
void N_verde::llenarDatos(char * paquete,int size){
    ...
    this->nombre = *id;
		...
		if(size > 17){
			...
			cout<<"Mi nombre es: " << this->nombre << " mi vecino es: " << nodo.nombre << endl;
		}


}

vector<Nodos> N_verde::getVecinos(){
	return vecinos;
}

void N_verde::setVecino(int ID, char * IP, int puerto){
	for(int i = 0; i < vecinos.size(); i++){
		if(vecinos[i].nombre == ID){
			copiar(IP,vecinos[i].IP,strlen(IP));
			vecinos[i].puerto = puerto;
			break;
		}
	}
}
